...
## 1.1 Khung Cấu trúc Nhân quả (Causal Framework)
...
### 1.1.1 Đồ thị có hướng không chu trình (Directed Acyclic Graph - DAG)
...
### 1.1.2 Mô hình Phương trình Cấu trúc (Structural Equation Models - SEM)
...
### 1.1.3 Thống kê quan sát và Can thiệp thực nghiệm (Do-Calculus)
...
## 1.2 Mô hình Nhiễu Cộng (Additive Noise Models - ANM)
...
### 1.2.1 Khái niệm Nhiễu Cộng Phi tuyến (Nonlinear ANM)
...
### 1.2.2 Tính Bất đối xứng của ANM (ANM Asymmetry)
...
## 1.3 Tiêu chuẩn Độc lập Thống kê Hilbert-Schmidt (HSIC)
...
### 1.3.1 Công thức Toán học của HSIC
...
### 1.3.2 Khắc phục giới hạn bằng Random Fourier Features (RFF)
...
## 1.4 Bài toán Khai phá Cấu trúc Liên tục (Continuous DAG)
...
### 1.4.1 Ảnh hưởng của thuật toán NOTEARS
...
### 1.4.2 Hàm phạt thế hệ thứ ba - DAGMA
...
## 1.5 Phương pháp Lọc Trọng số Cạnh (Edge Pruning)
...
### 1.5.1 Kỹ thuật Adaptive LASSO phi tuyến tĩnh
...
But LASSO rất tệ ở việc lọc cạnh có tỷ lệ đa cộng tuyến (Multicollinearity). Thuật toán **Adaptive LASSO (Zou, 2006)** đưa ra giải pháp cân bằng bằng cách trừng phạt các biến phụ thuộc dựa trên nghịch đảo độ quan trọng sơ khởi thông qua ma trận $\beta_{\text{init}}$:

$$ \arg\min_{\beta} \left( ||y - X\beta||^2_2 + \lambda \sum_j \frac{1}{|\hat{\beta}_{\text{init}, j}|^\gamma} |\beta_j| \right) $$

### 1.5.2 Phân tích tầm quan trọng bằng Random Forest (Permutation Importance)
...
### 1.5.3 Lọc thông tin gián tiếp qua Độ tương quan Điều kiện phần dư
